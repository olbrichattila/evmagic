...
func (h *handler) setHandlerConf() error {
	...
	subscriberConfig := sns.SubscriberConfig{
		...
		GenerateSqsQueueName: func(ctx context.Context, topic sns.TopicArn) (string, error) {
	...
	h.subscriber, err = sns.NewSubscriber(subscriberConfig, sqsSubscriberConfig, logger)
	if err != nil {
		return err
	}
